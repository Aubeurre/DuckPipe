...
namespace DuckPipe
{
    public partial class AssetManagerForm : Form
    {
        ...
        private async void LoadProductionList()
        {
            string rootPath = GetProductionRootPath();
            ...
            try
            {
                var folders = await Program.Storage.ListFoldersAsync(rootPath);

                foreach (var folder in folders)
                {
                    cbProdList.Items.Add(folder);
                }
            ...
            {
                MessageBox.Show("Erreur de chargement des productions : " + ex.Message);
            }
        }
